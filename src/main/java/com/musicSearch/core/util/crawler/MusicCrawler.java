...
public class MusicCrawler {
    ...
    public int getPageRange() throws IOException {
        ...
        crawling(mainUrl, new DOMOperation(){
            ...
            @Override
            public void iterate(Document doc) {
            	Element lastPageElement = doc.select(".pagination .lastpage").first();
    	...
    	for (int i=1; i<=target; i++) {
    		...
    		crawling(url + i, new DOMOperation(){
				...
				public void iterate(Document doc) {
					...
					Elements titleEle = doc.select("#freeTracks li .trackTitle");
    	...
    	for (String link : links) {
    		...
    		crawling(domain + link, new DOMOperation(){
				...
				public void iterate(Document doc) {
					...
					Element titleEle = doc.select(".page-head h1 span:first-child").first();
					String title = titleEle.text();
					Element sourceEle = doc.select(".ecommerce-actions a.ecommerce-btn-text").first();
